<script setup>
  import { defineProps } from 'vue';
  import { useCartStore } from '../store/CartStore';
  import { useComparisonStore } from '../store/ComparisonStore';
  import { useWishlistStore } from '../store/WishlistStore';
  import { computed } from 'vue';

  const cartStore = useCartStore();
  const comparisonStore = useComparisonStore();
  const wishlistStore = useWishlistStore();

  const addToCart = (product) => {
    cartStore.addToCart(product);
  };

  const addToComparison = (product) => {
    comparisonStore.addToComparison(product);
  };

  const isInWishlist = computed(() =>
    wishlistStore.wishlist.some(item => item.id === props.id)
  );

  const toggleWishlist = () => {
    if (isInWishlist.value) {
      wishlistStore.removeFromWishlist(props.id);
    } else {
      wishlistStore.addToWishlist(props);
    }
  };

  const props = defineProps({
    id: Number,
    title: String,
    description: String,
    image: String,
    price: Number,
    rating: Object,
    category: String,
  });
</script>

<template>
    <div class="flex flex-col max-h-[130rem] cursor-pointer max-w-80 hover:-translate-y-1 hover:scale-105 duration-300 bg-white border border-slate-200 shadow shadow-slate-950/5 rounded-2xl overflow-hidden">
      <router-link :to="`/products/${id}`">
        <img class="w-full object-contain h-48 mt-3" :src="image" alt="Product" />
      </router-link>
        <div class="flex-1 flex flex-col p-6">
          <header class="mb-2 flex-2">
            <h2 class="text-lg line-clamp-2 font-extrabold leading-snug">
              <div class="text-slate-600 focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300">
                {{ title }}
              </div>
            </h2>
          </header>
          
          <div class="flex mt-1 space-x-2">
            <div class="text-base line-clamp-2 font-extrabold text-slate-500 leading-snug">
              <h2>R{{ price }}</h2>
            </div>

            <span class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10">
              {{ category }}
            </span>
          </div>
          
        
          <div class="flex mt-1 space-x-2">
            <div class="justify-end space-x-1">
              <button 
                @click="addToCart(props)"
                class="inline-flex justify-center whitespace-nowrap rounded-lg bg-cyan-700 px-2 py-2 text-sm font-medium text-white hover:bg-cyan-900 focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300 transition-colors"
              >
                Add To Cart
              </button>
              <button @click="toggleWishlist" >
                <svg
                  class="me-1.5 h-5 w-5 hover:fill-red-500"
                  :class=" isInWishlist ? 'fill-red-500' : 'fill-white'"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  fill="none"
                  viewBox="0 0 24 24"
                  transform="scale(1.6)"
                >
                  <path
                    stroke="currentColor"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M12.01 6.001C6.5 1 1 8 5.782 13.001L12.011 20l6.23-7C23 8 17.5 1 12.01 6.002Z"
                  />
                </svg>
              </button>
              <button @click="addToComparison(props)" class="inline-flex justify-center whitespace-nowrap rounded-lg bg-cyan-700 px-2 py-2 text-sm font-medium text-white hover:bg-cyan-900 focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300 transition-colors">Add to Comparison</button>
            </div>
          </div>
        </div>
      
    </div>
</template>
  
  